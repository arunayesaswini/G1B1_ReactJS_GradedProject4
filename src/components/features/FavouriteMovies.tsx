import React,{Component} from "react";
import IMovie from "../../model/IMovie";
import {LoadingStatus} from "../../model/types";
import LoadingIndicator from "../common/LoadingIndicator";
import { Alert } from "react-bootstrap";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faHeart} from "@fortawesome/free-solid-svg-icons";
import MoviesList from "../movies-list/MoviesList";
import {getFavouriteMoviesFromServer} from "../../services/movies";


type Props={};

type State={
    status : LoadingStatus,
    favouriteMovies : IMovie[],
    error? : Error | null

};

class FavouriteMovies extends Component<Props,State>{

    constructor (props : Props){
        super(props);
        this.state={
            status : 'LOADING',
            favouriteMovies : []
        };
    }

    render(){
        let el;
        const {status, favouriteMovies,error} = this.state;
        switch(status){
            case 'LOADING':
                el=(
                    <LoadingIndicator 
                    size="large"
                    message="We are fetching Movies. Please wait..."
                    />
                );
            break;
            case 'LOADED':
                    el=(
                        <main>
                            <section className="my-2">
                                <header style={{color: "crimson"}}>
                                    <h1>
                                        <center>
                                        <FontAwesomeIcon icon={faHeart} className="me-3" />
                                           Favourite Movies
                                        </center>
                                    </h1>
                                </header>
                                <hr/>
                                <body>
                                    <MoviesList movies={favouriteMovies}/>
                                </body>
                            </section>
                        </main>
                    );
            break;
            case 'ERROR_LOADING':
                el=(
                    <Alert variant="danger">
                        {error?.message}
                    </Alert>
                );                
            break;
            
        }
        return el;
    }

    async componentDidMount(){
        this.setState({
            status:'LOADING'
        });
        try {
            const data = await getFavouriteMoviesFromServer();
            this.setState(
                {
                    status:'LOADED',
                    favouriteMovies : data
                }
            );
            
        } catch (error) {
            this.setState({
                status : 'ERROR_LOADING',
                error : error as Error
            }
            );
        }
    }
    

}
export default FavouriteMovies;